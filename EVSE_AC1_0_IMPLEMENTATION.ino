...
#if AC1_0_EVRE
#include "src/evse_src/MCP23008.h"
...
extern uint8_t rfid_read_flag;

#if UPDATED_OTA_ENABLE
#include "src/evse_src/evse_ota.h"
// #include <esp_task_wdt.h>
// // 2min WDT
// #define WDT_TIMEOUT 120

extern uint8_t evse_ota_enable;
extern Preferences evse_ota_pref;
#endif
uint32_t connnection_type = 0;

extern volatile int relay_duty;
void setup()
{
	...
	mcp.pinMode(EMERGENCY_PIN, INPUT);		// emergency pin 2 as input
	mcp.pinMode(EARTH_PRESENCE_PIN, INPUT); // Earth_presence pin 3 as input
	mcp.pinMode(BUZZER_PIN, OUTPUT);		// Buzzer pin 4 as output
	mcp.pinMode(RELAY_DROP_PIN, OUTPUT);	// relay drop pin 6 as output
	mcp.pinMode(BATTERY_CONTROL, OUTPUT);
#endif
	...
	evse_preferences_ble.begin("connectivity", true);
	connnection_type = evse_preferences_ble.getUInt("conn_type", 0);
	evse_preferences_ble.end();

	evse_ota_pref.begin("evse_ota", true);
	evse_ota_enable = evse_ota_pref.getUChar("evse_ota_en", 0);
	evse_ota_pref.end();
	...
	if (evse_ota_enable)
	{
		...
		{
			evse_preferences_ble.begin("wifi_cred", true);
			evse_ble_ssid = evse_preferences_ble.getString("ssid", "");
			evse_ble_ssid_key = evse_preferences_ble.getString("KEY", "");
			evse_preferences_ble.end();
			// ota_checking();
#if WIFI_OTA_ENABLE
			Serial.println("[setup] WiFi OTA Checking, Current Firmware Version: " + String(EVSE_CHARGE_POINT_FIRMWARE_OTA_VERSION));
			// ADDR_LEDs *ptr_leds;
			// ptr_leds->ADDR_LEDs_Init();
			// ptr_leds->White_led();
			init_wifi_ota();
		...
		case CONNECT_GSM_ENABLED:
		{
			...
			// ota_checking();
			Serial.println("[setup] TINY GSM OTA Checking, Current Firmware Version: " + String(EVSE_CHARGE_POINT_FIRMWARE_OTA_VERSION));
			// ADDR_LEDs *ptr_leds;
			// ptr_leds->ADDR_LEDs_Init();
			// ptr_leds->White_led();
	...
	else
	{
		...
		Serial.println("New OTA Implemented by :Abhigna , Sai goud & Gopikrishna");
		...
		Serial.println("OTA Update request URI :  " + String(evse_fota_uri));
		// Serial.println("evse_fota_retries :  " + String(evse_fota_retries));
		Serial.println("Last firmware update_date :  " + String(evse_fota_date));
		...
		// if (connnection_type != CONNECT_WIFI_GSM_ETHERNET_DISABLE)
		// {
		evse_ble_commisioning();
		...
		if (relay_dropflag == 1)
		{
			ptr_evse_relay->evse_relay_pwm_0();
		}
#endif
...
void management_task(void* p)
{
...
#if AC1_0_EVRE
	mcp.digitalWrite(BATTERY_CONTROL, HIGH);
#else
	...
	while (1)
	{
		...
		// uint32_t lu32_emergency = analogRead(mcp.digitalRead(EMERGENCY_PIN));
		uint32_t lu32_emergency = mcp.digitalRead(EMERGENCY_PIN);
		// Serial.print("EMERGENCY_PIN  test: ");
		// Serial.println(lu32_emergency);
		// uint32_t lu32_emergency = 0;
#else
		...
		{
			// Serial.println("EMGY Not Faulted  entry ");

			if (EvseChargePointErrorCode_OtherError == EVSE_EmergencyStop)
			{
				// Serial.println("EMGY Not Faulted  entry 2 ");
		...
		if (lu8_gfci == 1)
		{
			// Serial.println("relay off called  gfci");
			ptr_evse_relay->evse_relay_off();
#if RELAY_DROP
			if (relay_dropflag == 1)
			{
		...
		// uint32_t lu32_earth_disconnect = analogRead(mcp.digitalRead(EARTH_PRESENCE_PIN));
		uint32_t lu32_earth_disconnect = mcp.digitalRead(EARTH_PRESENCE_PIN);
		// uint32_t lu32_earth_disconnect = 0;
#else
		uint32_t lu32_earth_disconnect = evse_earth.earth_read();
#endif

		// if (lu32_earth_disconnect < 2047)
		if (lu32_earth_disconnect == 1)
		{
...
void ControlPilot(void* p)
{
...
#if EVSE_RELAY_WELD_DETECTION
	ptr_evse_relay->evse_relay_weld_detection_init();
#endif
	...
	ptr_cp_in->read_cp_init();
#if RELAY_DROP
	ptr_evse_relay->evse_relay_pwm_init();
#endif
	...
	// static int relay_duty = 100;
	int relay_duty_old = 0;
	uint32_t relay_time = 1;
	int rley_flag = 0;
	while (1)
	{
#if RELAY_DROP 
		if ((relay_dropflag == 1) && (timer <= 750))
		{
			if (timer++ == 100)
			{
				relay_duty = 75;
				ptr_evse_relay->evse_relay_pwm_75();
			}

			else if (timer == 750)
			{
				relay_duty = 50;
				ptr_evse_relay->evse_relay_pwm_50();
			}

			// else if ((timer == 1500))
			// {
			// 	relay_duty = 25;
			// 	ptr_evse_relay->evse_relay_pwm_25();
			// }
#if 0
#endif
		}
...
#if EVSE_RELAY_WELD_DETECTION
		uint8_t rleayweld = ptr_evse_relay->evse_relay_weld_detection();
#endif
			...
			(gs32EVSE_SMW_Control_pilot_Average > evse_state_a_lower_threshold))
		{
			PILOT_reading = V_12;
			EVSE_states = STATE_A;
			// Serial.println("relay off called state A");
			ptr_evse_relay->evse_relay_off();
#if RELAY_DROP
			if (relay_dropflag == 1)
			{
...
#endif
			// Serial.println("relay off called b");
			ptr_evse_relay->evse_relay_off();
#if RELAY_DROP
			if (relay_dropflag == 1)
			{
			...
			(gs32EVSE_SMW_Control_pilot_Average > evse_state_sus_lower_threshold)) // 1750 working
		{
			...
			PILOT_reading = V_SUS;
			Serial.println("relay off called sus");
			ptr_evse_relay->evse_relay_off();
#if RELAY_DROP
			if (relay_dropflag == 1)
			{
			...
			(gs32EVSE_SMW_Control_pilot_Average > evse_state_e2_lower_threshold))
		{
			...
			EVSE_states = STATE_E;
			Serial.println("relay off called state e");
			ptr_evse_relay->evse_relay_off();
#if RELAY_DROP
			if (relay_dropflag == 1)
			{
			...
			(gs32EVSE_SMW_Control_pilot_Average > evse_state_dis_lower_threshold)) // 1750 working
		{
			...
			// Serial.println("relay off called dis");
			ptr_evse_relay->evse_relay_off();
#if RELAY_DROP
			if (relay_dropflag == 1)
			{
			...
			(gs32EVSE_SMW_Control_pilot_Average > evse_state_c_lower_threshold))
		{
			...
			{
#if RELAY_DROP
				// ptr_evse_relay->SetControlPilotDuty(relay_duty);
				// ptr_evse_relay->evse_relay_pwm_on();
				// ptr_evse_relay->evse_relay_on();
				if (relay_duty == 100)
				{
					ptr_evse_relay->evse_relay_pwm_100();
				}
#else
				ptr_evse_relay->evse_relay_on();
#endif
			...
			{
				Serial.println("relay off called faulted");
				ptr_evse_relay->evse_relay_off();
#if RELAY_DROP
				if (relay_dropflag == 1)
				{
			...
			(gs32EVSE_SMW_Control_pilot_Average > evse_state_d_lower_threshold))
		{
			...
			{
				Serial.println("relay off called faulted2");
				ptr_evse_relay->evse_relay_off();
#if RELAY_DROP
				if (relay_dropflag == 1)
				{
		...
		else if (gs32EVSE_SMW_Control_pilot_Average < evse_state_e_threshold)
		{
			PILOT_reading = V_UNKNOWN;
			EVSE_states = STATE_E;
			Serial.println("relay off called state e2");
			ptr_evse_relay->evse_relay_off();
#if RELAY_DROP
			if (relay_dropflag == 1)
			{
		...
		else
		{
			...
			Serial.println("relay off called default");
...
#if AC1_0_EVRE
		evse_over_current_threshold = (cpcurrentlimit + 15);
#else
...
void ocpp_task(void* p)
{
	...
	while (1)
	{
				...
				heartbeat->setOnReceiveConfListener([](JsonObject payload)
					{
						...
						if (evse_ongoingTxn)
						{
							...
							if (sessionresume == 1)
							{
								...
								{
									EvseDevStatus_connector_1 = flag_EVSE_Request_Charge; // HI1
		...
		case flag_EVSE_Request_Charge:
		{
			...
			if (evse_ChargePointStatus == Faulted)
			{
				Serial.println("relay off called faulted3");
				ptr_evse_relay->evse_relay_off();
#if RELAY_DROP
				if (relay_dropflag == 1)
				{
		...
		case flag_EVSE_Stop_Transaction:
		{
			...
			case EVSE_STOP_TXN_INITIATED:
			{
				...
				// Serial.println("relay off called stop txn");
				ptr_evse_relay->evse_relay_off();
#if RELAY_DROP
				if (relay_dropflag == 1)
				{
